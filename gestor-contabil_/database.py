...
def criar_tabelas():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS produtos (
            ...
            icms_credito REAL NOT NULL,
            icms_debito REAL NOT NULL,
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS contas_gerais (
            nome_conta TEXT PRIMARY KEY,
...
def adicionar_fornecedor(nome, cnpj, cidade, estado):
    executar_query(
        "INSERT INTO fornecedores (nome, cnpj, cidade, estado) VALUES (?, ?, ?, ?)",
        (nome, cnpj, cidade, estado)
    )

def adicionar_cliente(nome, cpf_cnpj, cidade, estado):
    executar_query(
        "INSERT INTO clientes (nome, cpf_cnpj, cidade, estado) VALUES (?, ?, ?, ?)",
        (nome, cpf_cnpj, cidade, estado)
    )

def adicionar_produto(nome, marca, origem, preco_compra, preco_venda, icms_credito, icms_debito, id_fornecedor, estoque_inicial):
    executar_query("""
        INSERT INTO produtos (nome, marca, origem, preco_compra, preco_venda, icms_credito, icms_debito, id_fornecedor, estoque)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (nome, marca, origem, preco_compra, preco_venda, icms_credito, icms_debito, id_fornecedor, estoque_inicial))

def adicionar_venda(id_produto, id_cliente, quantidade, total_venda, tipo_pagamento, data_venda, parcelas=1):
    executar_query("""
        INSERT INTO vendas (id_produto, id_cliente, quantidade, total_venda, tipo_pagamento, data_venda, parcelas)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (id_produto, id_cliente, quantidade, total_venda, tipo_pagamento, data_venda, parcelas))
