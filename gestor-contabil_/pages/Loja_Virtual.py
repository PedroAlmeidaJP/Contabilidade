                    ...
                    with st.expander("Comprar este item"):
                        with st.form(key=f"form_compra_{produto['id']}", clear_on_submit=True):
                            ...
                            if not opcoes_clientes:
                                ...
                                id_cliente = st.selectbox("Selecione o Cliente", options=list(opcoes_clientes.keys()), format_func=lambda x: opcoes_clientes[x], key=f"cliente_{produto['id']}")
                                quantidade = st.number_input("Quantidade", min_value=1, max_value=int(produto['estoque']), step=1, key=f"qtd_{produto['id']}")
                                
                                # --- MUDANÇA AQUI: Opção de Pagamento ---
                                tipo_pagamento = st.selectbox("Forma de Pagamento", ["À Vista", "A Prazo"], key=f"pagamento_{produto['id']}")

                                submitted = st.form_submit_button("Finalizar Compra")
                                if submitted:
                                    total_venda = produto['preco_venda'] * quantidade
                                    db.adicionar_venda(produto['id'], id_cliente, quantidade, total_venda)
                                    novo_estoque = produto['estoque'] - quantidade
                                    db.executar_query("UPDATE produtos SET estoque = ? WHERE id = ?", (novo_estoque, produto['id']))

                                    # --- Lógica Contábil da Venda ---
                                    if tipo_pagamento == "À Vista":
                                        caixa_atual = db.get_conta_geral("caixa")
                                        db.update_conta_geral("caixa", caixa_atual + total_venda)
                                    else: # A Prazo
                                        # Pega o ID da última venda inserida para vincular
                                        id_venda = db.executar_query("SELECT last_insert_rowid()")[0]
